#include "wms/tile_provider_base.h"
...
tile_provider_base::tile_provider_base()
    : work_(asio_)
    , runner_thread_(boost::bind(&boost::asio::io_service::run, &asio_))
    , converter_(wms::get_png_converter())
    , debug_(NULL)
{
/*
...
shared_ptr<const tile_t> tile_provider_base::request_tile(const tile_id_t &id)
{
    // THREAD: client
    ...
    for (auto it = loaders_.begin(); it != loaders_.end(); ++it)
    {
        ...
        auto callback = boost::bind(&tile_provider_base::png_ready_callback, this, id, tile, _1, *it);
...
void tile_provider_base::png_ready_callback(const tile_id_t &id, shared_ptr<tile_t> tile, shared_ptr<const png_t> png, shared_ptr<png_provider> source_provider)
{
    // THREAD: service 
...
void tile_provider_base::add_provider(shared_ptr<png_provider> provider)
{
