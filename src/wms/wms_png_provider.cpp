...
{
public:
    request(const tile_id_t &id, boost::asio::io_service &io, const string &host, const string &path, png_callback final_callback);
    ~request();
...
wms_png_provider::request::request(const tile_id_t &id, boost::asio::io_service &io, const string &host, const string &path, png_callback final_callback)
    : host_(host)
    , path_(path)
    , tile_id_(id)
    , resolver_(io)
...
void wms_png_provider::request::start(shared_ptr<wms_png_provider::request> myself)
{
    ...
    request_stream << "?LAYERS=osm&SERVICE=mapnik&VERSION=1.1.1&REQUEST=GetMap&BBOX=";
...
wms_png_provider::wms_png_provider(const string &host, const string &path, boost::asio::io_service &io)
    : host_(host)
    , path_(path)
    , io_(&io)
{
...
{
    shared_ptr<request> r (new request(id, *io_, host_, path_, callback));
    r->start(r);
